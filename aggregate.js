 ...
 * @param {*} filePath
 */
var filePath = 'data/datafile.csv';
const fs = require('fs');
...
const aggregate = (filePath) => {
  ...
  const fileContentJSONObj = arrToObject
    (fileContentArray);

  function continent(countryArray) {
    var GDP = 0;
    var population = 0;
    for (let i = 0; i < (fileContentJSONObj.length); i++) {
  ...
  const mapFile = [continent(southAmericaCountries), continent(oceaniaCountries), continent(northAmericaCountries), continent(asiaCountries), continent(europeCountries), continent(africaCountries)];

  for (let i = 0; i < continentList.length; i++) {
    output[continentList[i]].GDP_2012 = mapFile[i][0];
  }

  for (let i = 0; i < continentList.length; i++) {
    output[continentList[i]].POPULATION_2012 = mapFile[i][1];
  }
  console.log(output);
